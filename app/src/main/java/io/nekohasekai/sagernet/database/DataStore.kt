...
object DataStore : OnPreferenceDataStoreChangeListener {
    ...
    var logLevel by configurationStore.stringToInt(Key.LOG_LEVEL) { 2 } // default: info (0=none disables core logs entirely)
    ...
    var serverMultiplexing by profileCacheStore.string(Key.SERVER_MULTIPLEXING)
    var serverTrafficPattern by profileCacheStore.string(Key.SERVER_TRAFFIC_PATTERN)
    ...
    var serverUDPOverStream by profileCacheStore.boolean(Key.SERVER_UDP_OVER_STREAM)
    var serverHeartbeat by profileCacheStore.string(Key.SERVER_HEARTBEAT)
    var serverIdleSessionCheckInterval by profileCacheStore.string(Key.SERVER_IDLE_SESSION_CHECK_INTERVAL)
    var serverIdleSessionTimeout by profileCacheStore.string(Key.SERVER_IDLE_SESSION_TIMEOUT)
    var serverMinIdleSession by profileCacheStore.stringToInt(Key.SERVER_MIN_IDLE_SESSION)
    var serverHostKeyAlgorithms by profileCacheStore.string(Key.SERVER_HOST_KEY_ALGORITHMS)
    var serverClientVersion by profileCacheStore.string(Key.SERVER_CLIENT_VERSION)
    var serverPersistentKeepalive by profileCacheStore.stringToInt(Key.SERVER_PERSISTENT_KEEPALIVE)
    ...
    var routeGateway by profileCacheStore.boolean(Key.ROUTE_GATEWAY)
